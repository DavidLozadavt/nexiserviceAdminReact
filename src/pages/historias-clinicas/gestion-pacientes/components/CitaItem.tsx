...
export const CitaItem: React.FC<CitaItemProps> = ({ cita, onClick }) => {
  const getTipoCitaColor = (tipo: CitaPaciente['tipoCita']) => {
    switch (tipo) {
      ...
      default:
        return 'bg-gray-100 dark:bg-gray-400 text-gray-600 dark:text-gray-100';
    }
  };
  ...
  return (
    <button
      ...
      className="w-full bg-white dark:bg-coal-300 hover:bg-gray-100 dark:hover:bg-coal-200 border-2 border-gray-200 dark:border-coal-100 hover:border-primary rounded-xl p-4 transition-all duration-200 text-left group hover:shadow-lg dark:hover:shadow-none"
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex-shrink-0">
          <div className="w-16 h-16 bg-gradient-to-br from-primary to-primary-active rounded-xl flex flex-col items-center justify-center shadow-primary dark:shadow-none">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-2">
            <div className="flex-1 min-w-0">
              <h4 className="text-2sm font-semibold text-gray-900 dark:text-gray-100 truncate group-hover:text-primary transition-colors">
                {cita.pacienteNombre}
              </h4>
              <p className="text-2xs text-gray-600 dark:text-gray-700 font-medium">
          ...
          {cita.motivoConsulta && (
            <p className="text-2xs text-gray-600 dark:text-gray-700 line-clamp-2">
              {cita.motivoConsulta}
            </p>
          )}
        </div>
        ...
        <div className="flex-shrink-0 self-center">
          <svg className="w-5 h-5 text-gray-400 dark:text-gray-600 group-hover:text-primary group-hover:translate-x-1 transition-all" fill="none" stroke="currentColor" viewBox="0 0 24 24">
