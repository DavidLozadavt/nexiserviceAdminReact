...
const GestionRutaContent = ({ reload }: ContentProps) => {
  ...
  const storageFilterId = 'rutas-filter';
  ...
  const filteredData = useMemo(() => {
    ...
    return rutas.filter(
      (ruta) =>
        ...
        ruta.tiempoEstimado.toString().includes(searchTerm.toLowerCase()) ||
        ruta.origen?.descripcion?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        ruta.destino?.descripcion?.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [searchTerm, rutas]);
