...
const ModalDetailVehiculo = ({ open, data, onClose, onSave }: ModalProps) => {
      ...
      <div key={index} className="flex items-center gap-3">
        <div className="flex items-center grow gap-2.5">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs text-gray-700">Vigencia hasta: {item.date}</span>
          </div>
        </div>
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <ModalContent className="max-w-[680px] top-[5%] p-4">
        <ModalHeader>
          <ModalTitle>Vehículo {data?.placa || 'N/A'}</ModalTitle>
