...
const TipoIncapacidadesContent = ({ reload }: ContentProps) => {
  ...
  const columns = useMemo<ColumnDef<any>[]>(
    () => [
      ...
      {
        accessorFn: (row) => row.porcentajeDePago,
        id: 'porcentajeDePago',
        header: () => 'Porcentaje de Pago',
        enableSorting: true,
        cell: (info) => (
          <span className="text-gray-700">{info.row.original.porcentajeDePago}%</span>
        ),
        meta: {
          className: 'min-w-[150px]',
          cellClassName: 'text-gray-700 font-normal'
  ...
  const filteredData = useMemo(() => {
    ...
    return incapacidades.filter(
      (centroC) =>
        ...
        centroC.porcentajeDePago.toString().includes(searchTerm) ||
