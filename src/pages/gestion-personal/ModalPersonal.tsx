...
const ModalPersonal = ({ open, persona, onClose, onSave }: ModalProps) => {
  ...
  useEffect(() => {
    if (open) {
      if (persona) {
        const p = persona.persona || {}; // los datos reales están dentro de persona.persona
        setPrimerNombre(p.nombre1 || '');
        setSegundoNombre(p.nombre2 || '');
        setPrimerApellido(p.apellido1 || '');
        setSegundoApellido(p.apellido2 || '');
        setIdentificacion(p.identificacion || '');
        setFechaNacimiento(p.fechaNac || '');
