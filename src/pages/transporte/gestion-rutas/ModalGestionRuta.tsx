...
const ModalGestionRuta = ({ open, onClose, data, onSave }: ModalProps) => {
  ...
  const [idDepartamentoOrigen, setIdDepartamentoOrigen] = useState(data?.idDepartamentoOrigen || '');
  const [idCiudadOrigen, setIdCiudadOrigen] = useState(data?.idCiudadOrigen || '');
  const [idDepartamentoDestino, setIdDepartamentoDestino] = useState(data?.idDepartamentoDestino || '');
  ...
  const fetchCiudadesPorDepartamento = async (departamentoId: string, tipo: 'origen' | 'destino') => {
    try {
  ...
  const validate = () => {
    ...
    if (!idDepartamentoOrigen) newErrors.idDepartamentoOrigen = 'El departamento de origen es requerido.';
    if (!idCiudadOrigen) newErrors.idCiudadOrigen = 'La ciudad de origen es requerida.';
    if (!idDepartamentoDestino) newErrors.idDepartamentoDestino = 'El departamento de destino es requerido.';
  ...
  const handleSave = async () => {
    ...
    try {
      if (data) {
        await axios.put(`rutas/${data.id}`, payload);
        ...
        <div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (errors.idDepartamentoOrigen) setErrors((prev) => ({ ...prev, idDepartamentoOrigen: '' }));
              }}
            >
            ...
            {errors.idDepartamentoOrigen && <p className="mt-1 text-sm text-red-500">{errors.idDepartamentoOrigen}</p>}
          </div>
          ...
          <div>
            ...
            {errors.idCiudadOrigen && <p className="mt-1 text-sm text-red-500">{errors.idCiudadOrigen}</p>}
          </div>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (errors.idDepartamentoDestino) setErrors((prev) => ({ ...prev, idDepartamentoDestino: '' }));
              }}
            >
              <option value="">Seleccione un departamento</option>
              {departamentos.map((departamento: any) => (
            ...
            {errors.idDepartamentoDestino && <p className="mt-1 text-sm text-red-500">{errors.idDepartamentoDestino}</p>}
          </div>
          ...
          <div>
            ...
            {errors.idCiudadDestino && <p className="mt-1 text-sm text-red-500">{errors.idCiudadDestino}</p>}
          </div>
          <div>
          ...
          <div>
            ...
            />
            {errors.tiempoEstimado && <p className="mt-1 text-sm text-red-500">{errors.tiempoEstimado}</p>}
          </div>
    <div>
